...
public class Dependencies {
	...
	public static void init() {
		try {
			...
			if (!OtherGrowthConfig.globalDisableMetrics) enableMetrics();
			worldGuard = (WorldGuardPlugin)getPlugin("WorldGuard");
			
			if (!foundPlugins.isEmpty()) Log.normal("Found plugin(s): '"+foundPlugins+"'");
			if (!notFoundPlugins.isEmpty()) Log.high("Plugin(s) not found: '"+notFoundPlugins+"' (OtherGrowth will continue to load)");
		} catch (Exception e) {
	...
	public static Plugin getPlugin(String name) {
		Plugin plugin = OtherGrowth.plugin.getServer().getPluginManager().getPlugin(name);
